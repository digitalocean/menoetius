	...
	{                                                                                              \
		...
		while( _stacki_ > 0 ) {                                                                    \
			...
			while( _end_ - _start_ > 2 ) {                                                         \
				...
				_j_ = _end_ - 1;                                                                   \
				while( _i_ < _j_ ) {                                                               \
					for( ; _i_ <= _j_ && comparator( ( ( a )[_i_] ), ( ( a )[_p_] ) ) <= 0;        \
						 _i_++ )                                                                   \
						;                                                                          \
					if( _i_ > _j_ ) {                                                              \
						...
						for( ; _i_ <= _j_ && comparator( ( ( a )[_j_] ), ( ( a )[_p_] ) ) >= 0;    \
							 _j_-- )                                                               \
							;                                                                      \
						if( _i_ > _j_ ) {                                                          \
...
#define SGLIB_ARRAY_BINARY_SEARCH(                                                                 \
	type, a, start_index, end_index, key, comparator, found, result_index )                        \
	{                                                                                              \
...
#define SGLIB_HASH_TAB_ADD_IF_NOT_MEMBER(                                                          \
	type, table, dim, elem, hash_function, comparator, member )                                    \
	{                                                                                              \
		unsigned _pos_;                                                                            \
		type* _elem_;                                                                              \
...
#define SGLIB_HASH_TAB_FIND_MEMBER(                                                                \
	type, table, dim, elem, hash_function, comparator, resultIndex, resultMember )                 \
	{                                                                                              \
		unsigned _i_;                                                                              \
		int _count_;                                                                               \
		type* _e_;                                                                                 \
		...
		_i_ %= ( dim );                                                                            \
		while( ( _e_ = ( table )[_i_] ) != NULL && comparator( _e_, ( elem ) ) != 0 &&             \
			   _count_ < ( dim ) ) {                                                               \
			_count_++;                                                                             \
...
#define SGLIB_HASH_TAB_MAP_ON_ELEMENTS(                                                            \
	type, table, dim, iteratedIndex, iteratedVariable, command )                                   \
	{                                                                                              \
...
#define SGLIB_LIST_DELETE_IF_MEMBER( type, list, elem, comparator, next, member )                  \
	{                                                                                              \
		type** _p_;                                                                                \
		for( _p_ = &( list ); *_p_ != NULL && comparator( ( *_p_ ), ( elem ) ) != 0;               \
			 _p_ = &( *_p_ )->next )                                                               \
			;                                                                                      \
		( member ) = *_p_;                                                                         \
		if( *_p_ != NULL ) {                                                                       \
...
#define SGLIB_SORTED_LIST_ADD( type, list, elem, comparator, next )                                \
	{                                                                                              \
		...
		SGLIB_SORTED_LIST_FIND_MEMBER_OR_PLACE(                                                    \
			type, list, elem, comparator, next, _cmpres_, _e_ );                                   \
		( elem )->next = *_e_;                                                                     \
		*_e_ = ( elem );                                                                           \
	}
...
#define SGLIB_SORTED_LIST_ADD_IF_NOT_MEMBER( type, list, elem, comparator, next, member )          \
	{                                                                                              \
		...
		int _cmp_res_;                                                                             \
		SGLIB_SORTED_LIST_FIND_MEMBER_OR_PLACE(                                                    \
			type, list, elem, comparator, next, _cmp_res_, _e_ );                                  \
		if( _cmp_res_ != 0 ) {                                                                     \
			( elem )->next = *_e_;                                                                 \
			*_e_ = ( elem );                                                                       \
			( member ) = NULL;                                                                     \
...
#define SGLIB_SORTED_LIST_FIND_MEMBER( type, list, elem, comparator, next, member )                \
	{                                                                                              \
		type* _p_;                                                                                 \
		int _cmpres_ = 1;                                                                          \
		for( _p_ = ( list ); _p_ != NULL && ( _cmpres_ = comparator( _p_, ( elem ) ) ) < 0;        \
			 _p_ = _p_->next )                                                                     \
			;                                                                                      \
		if( _cmpres_ != 0 )                                                                        \
			( member ) = NULL;                                                                     \
...
#define SGLIB_SORTED_LIST_FIND_MEMBER_OR_PLACE(                                                    \
	type, list, elem, comparator, next, comparator_result, member_ptr )                            \
	{                                                                                              \
		( comparator_result ) = -1;                                                                \
		for( ( member_ptr ) = &( list );                                                           \
			 *( member_ptr ) != NULL &&                                                            \
			 ( ( comparator_result ) = comparator( ( *member_ptr ), ( elem ) ) ) < 0;              \
...
#define SGLIB___DL_LIST_GENERIC_ADD_IF_NOT_MEMBER(                                                 \
	type, list, elem, comparator, previous, next, member, the_add_operation )                      \
	{                                                                                              \
		type* _dlp_;                                                                               \
		for( _dlp_ = ( list ); _dlp_ != NULL && comparator( _dlp_, ( elem ) ) != 0;                \
...
#define SGLIB_DL_LIST_ADD_BEFORE_IF_NOT_MEMBER(                                                    \
	type, list, elem, comparator, previous, next, member )                                         \
	{                                                                                              \
		SGLIB___DL_LIST_GENERIC_ADD_IF_NOT_MEMBER(                                                 \
			type, list, elem, comparator, previous, next, member, SGLIB_DL_LIST_ADD_BEFORE );      \
	}

#define SGLIB_DL_LIST_ADD_AFTER_IF_NOT_MEMBER(                                                     \
	type, list, elem, comparator, previous, next, member )                                         \
	{                                                                                              \
		SGLIB___DL_LIST_GENERIC_ADD_IF_NOT_MEMBER(                                                 \
			type, list, elem, comparator, previous, next, member, SGLIB_DL_LIST_ADD_AFTER );       \
	}

#define SGLIB_DL_LIST_ADD_IF_NOT_MEMBER( type, list, elem, comparator, previous, next, member )    \
	{                                                                                              \
		SGLIB___DL_LIST_GENERIC_ADD_IF_NOT_MEMBER(                                                 \
			type, list, elem, comparator, previous, next, member, SGLIB_DL_LIST_ADD );             \
	}
...
#define SGLIB___BIN_TREE_MAP_ON_ELEMENTS(                                                          \
	type, tree, iteratedVariable, order, left, right, command )                                    \
	{                                                                                              \
...
#define SGLIB_BIN_TREE_MAP_ON_ELEMENTS( type, tree, _current_element_, left, right, command )      \
	{                                                                                              \
		SGLIB___BIN_TREE_MAP_ON_ELEMENTS(                                                          \
			type, tree, _current_element_, 1, left, right, command );                              \
	}

#define SGLIB_BIN_TREE_MAP_ON_ELEMENTS_PREORDER(                                                   \
	type, tree, _current_element_, left, right, command )                                          \
	{                                                                                              \
		SGLIB___BIN_TREE_MAP_ON_ELEMENTS(                                                          \
			type, tree, _current_element_, 0, left, right, command );                              \
	}

#define SGLIB_BIN_TREE_MAP_ON_ELEMENTS_POSTORDER(                                                  \
	type, tree, _current_element_, left, right, command )                                          \
	{                                                                                              \
		SGLIB___BIN_TREE_MAP_ON_ELEMENTS(                                                          \
			type, tree, _current_element_, 2, left, right, command );                              \
	}
...
#define SGLIB_DEFINE_HEAP_PROTOTYPES(                                                              \
	heap_type, elem_type, afield, ifield, dim, comparator, elem_exchanger )                        \
...
#define SGLIB_DEFINE_HEAP_FUNCTIONS(                                                               \
	heap_type, elem_type, afield, ifield, dim, comparator, elem_exchanger )                        \
	...
	{                                                                                              \
		SGLIB_HEAP_DELETE_FIRST(                                                                   \
			elem_type, q->afield, q->ifield, dim, comparator, elem_exchanger );                    \
	}                                                                                              \
...
#define SGLIB_DEFINE_HASHED_TABLE_PROTOTYPES( type, dim, hash_function, comparator )               \
	...
	extern int sglib_hashed_##type##_add_if_not_member(                                            \
		type* table[dim], type* elem, type** member );                                             \
	...
	extern type* sglib_hashed_##type##_it_init_on_equal(                                           \
		struct sglib_hashed_##type##_iterator* it,                                                 \
		type* table[dim],                                                                          \
		int ( *subcomparator )( type*, type* ),                                                    \
		type* equalto );                                                                           \
...
#define SGLIB_DEFINE_HASHED_TABLE_FUNCTIONS( type, dim, hash_function, comparator )                \
	...
	{                                                                                              \
		SGLIB_HASH_TAB_ADD_IF_NOT_MEMBER(                                                          \
			type, table, dim, elem, hash_function, comparator, *member );                          \
	}                                                                                              \
...
#define SGLIB_DEFINE_HASHED_CONTAINER_PROTOTYPES( type, dim, hash_function )                       \
	...
	extern int sglib_hashed_##type##_delete_if_member(                                             \
		type* table[dim], type* elem, type** memb );                                               \
	extern int sglib_hashed_##type##_is_member( type* table[dim], type* elem );                    \
	extern type* sglib_hashed_##type##_find_member( type* table[dim], type* elem );                \
	extern type* sglib_hashed_##type##_it_init( struct sglib_hashed_##type##_iterator* it,         \
												type* table[dim] );                                \
...
#define SGLIB_DEFINE_HASHED_CONTAINER_FUNCTIONS( type, dim, hash_function )                        \
	...
	{                                                                                              \
		...
		it->subcomparator = subcomparator;                                                         \
		it->equalto = equalto;                                                                     \
		e = sglib_##type##_it_init_on_equal(                                                       \
			&it->containerIt, table[it->currentIndex], it->subcomparator, it->equalto );           \
		if( e == NULL )                                                                            \
	...
	{                                                                                              \
		...
		while( e == NULL && ( ++( it->currentIndex ) ) < ( dim ) ) {                               \
			e = sglib_##type##_it_init_on_equal(                                                   \
				&it->containerIt, it->table[it->currentIndex], it->subcomparator, it->equalto );   \
		}                                                                                          \
		return ( e );                                                                              \
	}
...
#define SGLIB___RBTREE_FIX_DELETION_DISCREPANCY(                                                   \
	type, tree, leftt, rightt, bits, RED, BLACK, res )                                             \
	{                                                                                              \
	...
	{                                                                                              \
		...
		if( t->right == NULL ) {                                                                   \
			*theLeaf = t;                                                                          \
			if( t->left != NULL ) {                                                                \
				if( SGLIB___GET_VALUE( t->bits ) == BLACK &&                                       \
					SGLIB___GET_VALUE( t->left->bits ) == BLACK )                                  \
					res = 1;                                                                       \
	...
	{                                                                                              \
		...
		if( t == NULL ) {                                                                          \
			...
			if( cmp < 0 || ( cmp == 0 && elem < t ) ) {                                            \
				...
				assert( elem == t && "Deleting an element which is non member of the tree, use "   \
				...
				if( t->left == NULL ) {                                                            \
					if( t->right == NULL ) {                                                       \
						...
						*tree = NULL;                                                              \
						...
						if( SGLIB___GET_VALUE( t->bits ) == 0 &&                                   \
							SGLIB___GET_VALUE( t->right->bits ) == 0 )                             \
							res = 1;                                                               \
				...
				if( it->pass[i] >= 2 ) {                                                           \
					...
					if( eqt != NULL ) {                                                            \
						if( subcomparator == NULL ) {                                              \
							SGLIB___BIN_TREE_FIND_MEMBER(                                          \
								type, s, eqt, left, right, comparator, s );                        \
						}                                                                          \
						else {                                                                     \
							SGLIB___BIN_TREE_FIND_MEMBER(                                          \
								type, s, eqt, left, right, subcomparator, s );                     \
						}                                                                          \
					}                                                                              \
					if( s != NULL ) {                                                              \
						j = i + 1;                                                                 \
	...
	{                                                                                              \
		...
		if( equalto == NULL ) {                                                                    \
			...
			if( subcomparator == NULL ) {                                                          \
				...
				SGLIB___BIN_TREE_FIND_MEMBER(                                                      \
					type, tree, equalto, left, right, subcomparator, t );                          \
			}                                                                                      \
		}                                                                                          \
		if( t == NULL ) {                                                                          \
			it->pathi = 0;                                                                         \
...
#define SGLIB_SAFE_REVERSE_NUMERIC_COMPARATOR( x, y )                                              \
	( ( ( x ) > ( y ) ? -1 : ( ( x ) < ( y ) ? 1 : 0 ) ) )
